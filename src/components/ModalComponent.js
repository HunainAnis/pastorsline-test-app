...
export const ModalComponent = ({ type, toggle, show, data, fetchMore, search, ...props }) => {
    ...
    const handleUpdate = (values) => {
        ...
        if (t > 1 && props.usContacts?.contacts.length!==0) {
                ...
                <Modal.Body>
                    {
                     ...
                     (   <>
                            <div className='d-block mx-auto mb-3 border-0'>
                                    <Button className='ml-1' style={{backgroundColor:"#46139f", border:"1px solid #46139f"}} onClick={()=>history.push("/ModalA")}>All Contacts</Button>
                                    <Button className='ml-1' style={{backgroundColor:"#ff7f50", border:"1px solid #ff7f50"}}  onClick={()=>history.push("/ModalB")}>US Contacts</Button>
                                    <Button className='ml-1' variant='white' style={{border:"1px solid #46139f"}} onClick={()=>history.push("/")}>Close</Button>
                            </div>
                            <div className='mb-3'>
                                <input placeholder='Search here...' className='w-100' type='text' value={searchText} onChange={(e)=> handleSearch(e.target.value)} />
                            </div>
                            {
                                <Scrollbars onUpdate={(values)=>handleUpdate(values)} style={{ height: 400 }}>
                                {/* <Scrollbars style={{ height: 400 }}> */}
                                    {
                                        contacts_ids.length!==0 && !onlyEven 
                                        ? 
                                        contacts_ids.map(i=>(
                                        ...
                                        :
                                        contacts_ids.filter(j=>j%2===0).map(i=>(
                                            <div onClick={()=>handleDetails(contacts[i])} className='my-1 border border-dark rounded p-2 cursor-pointer'  key={contacts[i].id}>
                                                {contacts[i].id}
                                            </div>
                                        ))
